...
class RoleService:
    ...
    def list(self, skip: int = 0, limit: int = 100, org_id: Optional[int] = None):
        logger.debug("list Role skip=%s limit=%s org_id=%s", skip, limit, org_id)
    ...
    def create(self, payload: RoleCreate, org_id: Optional[int] = None):
        logger.info("create Role org_id=%s", org_id)
        data = payload.model_dump()
        if org_id is not None:
            data["organisation_id"] = org_id
        return self._repo.create(data)

    def update(self, id: int, payload: RoleUpdate, org_id: Optional[int] = None):
        item = (
