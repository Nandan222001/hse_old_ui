...
class PermitToWorkService:
    ...
    def list(self, skip: int = 0, limit: int = 100, org_id: Optional[int] = None):
        logger.debug("list PermitToWork skip=%s limit=%s org_id=%s", skip, limit, org_id)
    ...
    def create(self, payload: PermitToWorkCreate, org_id: Optional[int] = None):
        logger.info("create PermitToWork org_id=%s", org_id)
        data = payload.model_dump()
        if org_id is not None:
            data["organisation_id"] = org_id
        return self._repo.create(data)

    def update(self, id: int, payload: PermitToWorkUpdate, org_id: Optional[int] = None):
        item = (
